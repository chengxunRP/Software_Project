...
/** User-friendly public status label from MySQL events.status */
function publicStatusLabel(status) {
  if (status === "Open") return "Open for registration";
  if (status === "Full") return "Full — waiting list available";
...
/** Existing badge classes — preserve colours (no new design). */
function publicStatusBadgeClass(status) {
  if (status === "Open") return "badge-cc-open";
  if (status === "Full") return "badge-cc-warning";
  if (status === "Closed") return "badge-cc-muted";
  if (status === "Cancelled") return "badge-cc-danger";
  if (status === "Completed") return "badge-cc-completed";
  return "badge-cc-muted";
}

/** Active upcoming catalogue: Open / Full / Closed, not started yet. */
const CATALOGUE_STATUS_SQL = "e.status IN ('Open', 'Full', 'Closed') AND e.start_datetime >= NOW()";

/** Public event-details may load these statuses (Draft never). */
const PUBLIC_DETAILS_STATUS_SQL = "e.status IN ('Open', 'Full', 'Closed', 'Cancelled', 'Completed')";
...
function formatPublicEvent(row) {
  ...
  return Object.assign({
    ...
    statusBadgeClass: publicStatusBadgeClass(row.status),
...
function buildCatalogueFilters(query) {
  ...
  const where = ["(" + CATALOGUE_STATUS_SQL + ")"];
  const params = [];
...
module.exports = {
  ...
  publicStatusBadgeClass: publicStatusBadgeClass
};
